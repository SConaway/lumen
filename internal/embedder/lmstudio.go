...
// that exposes an OpenAI-compatible /v1/embeddings endpoint. It delegates to
// OpenAI, which implements the same wire format, with no API key.
...
func NewLMStudio(model string, dimensions int, baseURL string) (*LMStudio, error) {
	o, err := NewOpenAI(model, dimensions, baseURL, "")
	if err != nil {
		return nil, err
	}
	return &LMStudio{OpenAI: o}, nil
}
